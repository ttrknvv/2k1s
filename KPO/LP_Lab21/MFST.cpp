...
namespace MFST
{
	...
	Mfst::Mfst(LEX plex, GRB::Greibach pgrebach)
	{
		grebach = pgrebach;
		lex = plex;
	...
	bool Mfst::start()
	{
		...
		case LENTA_END: MFST_TRACE4("------>LENTA_END")
			...
			std::cout << std::setw(4) << std::left << 0 << ": всего строк " << lex.lextable.table[lex.lextable.size - 1].sn << ", синтаксический анализ выполнен без ошибок" << std::endl;
			rc = true;
			break;
