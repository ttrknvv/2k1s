...
#define IP_DEST_HOST_UNREACHEBLE 11003
...
void Ping(const char* cHost, unsigned int Timeout, unsigned int RequestCount)
{
    ...
    PICMP_ECHO_REPLY pIpe = (PICMP_ECHO_REPLY)GlobalAlloc(GHND, sizeof(ICMP_ECHO_REPLY)
                            + sizeof(SendData)); // выделяем память под пакет - буфера ответа
    if (pIpe == 0)
    {
        IcmpCloseHandle(hIP);
        WSACleanup();
    ...
    {
        int dwStatus = IcmpSendEcho(hIP, ipaddr, SendData, sizeof(SendData), &option, pIpe,
                                    sizeof(ICMP_ECHO_REPLY) + sizeof(SendData), Timeout);
        if (dwStatus > 0)
        {
    ...
    cout << "Статистика Ping " << (int)*pByte << "." << (int)*((pByte + 1))
         << "." << (int)*((pByte + 2))
         << "." << (int)*((pByte + 3)) << endl;
    cout << "\tПакетов: отправлено = " << RequestCount << ", получено = "
         << RequestCount - LostPacketsCount << ", потеряно = "
         << LostPacketsCount << "<" << (int)(100 / (float)RequestCount) * LostPacketsCount
         << "% потерь>, " << endl;

    cout << "Приблизительное время приема- передачи: " << endl
         << "Минимальное = " << MinMS << "мс, Максимальное = " << MaxMS << "мс, "
         << "Среднее = " << (MaxMS + MinMS / 2)
         << "мс" << endl;
...
int main(int argc, char** argv)
{
    ...
    const char* first_parameter = argv[1];
    int second_parameter = atoi(argv[2]);
    int third_parameter = atoi(argv[3]);
    Ping(first_parameter, second_parameter, third_parameter);
    //Ping("192.168.43.189", 60, 10);
    system("pause");
}
